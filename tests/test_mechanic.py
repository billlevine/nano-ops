...
the incremental digest (section hashing, ledger cursor, snapshot lifecycle),
and the optional EXTRACTION lens.
Run: python3 tests/test_mechanic.py

Everything here runs against a tempdir estate — never real state/ — and the
fixtures invent a neutral installation. Nothing names an operator, a persona,
a channel, a host or an absolute path, and the identity tests assert exactly
that: the hub's session title and the agent-deck profile come out of the
fixture loops.toml, not out of the engine."""
import contextlib
...
SCRIPTS = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..",
                       "loops", "mechanic", ".claude", "skills", "mechanic",
                       "scripts")
sys.path.insert(0, os.path.abspath(SCRIPTS))
import mechanic  # noqa: E402

SCRIPT = os.path.abspath(os.path.join(SCRIPTS, "mechanic.py"))
...
class TestHubIdentity(unittest.TestCase):
    """Identity is configuration: the hub's session title and the agent-deck
    profile come out of loops.toml [hub], never out of this engine."""

    def test_defaults_are_neutral_when_hub_is_unnamed(self):
        self.assertEqual(mechanic.hub_identity({}), ("ops (hub)", "ops"))

    def test_persona_drives_the_title(self):
        title, profile = mechanic.hub_identity(
            {"hub": {"persona": "atlas", "deck_profile": "night"}})
        self.assertEqual((title, profile), ("atlas (hub)", "night"))

    def test_session_title_overrides_the_derived_title(self):
        title, _ = mechanic.hub_identity(
            {"hub": {"persona": "atlas", "session_title": "control plane"}})
        self.assertEqual(title, "control plane")

    def test_env_overrides_the_configured_profile(self):
        with mock.patch.dict(os.environ, {"MECHANIC_DECK_PROFILE": "scratch"}):
            _, profile = mechanic.hub_identity({"hub": {"deck_profile": "n"}})
        self.assertEqual(profile, "scratch")

    def test_blank_values_fall_back_rather_than_producing_an_empty_title(self):
        title, profile = mechanic.hub_identity(
            {"hub": {"persona": "  ", "session_title": "", "deck_profile": ""}})
        self.assertEqual((title, profile), ("ops (hub)", "ops"))
...
# The hub's title is derived, never spelled out: [hub] persona = "ops" in the
# fixture registry means "ops (hub)", exactly as bin/ops and bin/dashboard
# resolve it. A test that hardcoded a title would pass against an engine that
# hardcoded the same one.
HUB_TITLE = "ops (hub)"
DECK = {
    HUB_TITLE: {"title": HUB_TITLE, "status": "waiting",
...
class TestGatherDigest(unittest.TestCase):
    ...
    def test_non_autostart_loop_absence_is_not_drift(self):
        ...
        deck = {HUB_TITLE: {"title": HUB_TITLE, "status": "waiting",
                            "model_id": "claude-sonnet-5"}}
        env = {"MECHANIC_NOW": NOW_N1, "MECHANIC_REPO_ROOT": self.root,
               "MECHANIC_STATE_DIR": self.state}
        buf = io.StringIO()
...
class TestExtractionDigest(unittest.TestCase):
    def setUp(self):
        self.tmp = tempfile.TemporaryDirectory()
        self.addCleanup(self.tmp.cleanup)
        self.root = os.path.join(self.tmp.name, "fork")
