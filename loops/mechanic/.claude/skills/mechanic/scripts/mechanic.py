...
script for state I/O. Same split every loop in this estate uses: a script for
deterministic state, the skill for judgment.
...
$MECHANIC_DECK_PROFILE overrides the agent-deck profile (otherwise loops.toml's
[hub] deck_profile, falling back to "ops").
...
def hub_config(registry: dict) -> dict:
    """WHO THIS INSTALLATION'S HUB IS, from loops.toml's [hub] table.

    The core bakes in no operator identity: persona, session title and
    agent-deck profile all come from config, defaulting to the neutral "ops".
    Same resolution bin/doorbell and bin/dashboard use — keep them in step."""
    hub = registry.get("hub") or {}
    persona = hub.get("persona") or "ops"
    return {
        "persona": persona,
        # The agent-deck title convention is "<persona> (<loop name>)", and the
        # hub's loop name is "hub".
...
# --------------------------------------------------------------------------- #
# the EXTRACTION lens — operations -> nano-ops candidate detection
# --------------------------------------------------------------------------- #
# nano-ops is the public core this estate extracts into, built from an
